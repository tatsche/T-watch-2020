...
void init_timezones(void) {
  auto GMT_tz = zoneManager.createForZoneId(TZ_GMT);
  auto homeTz = zoneManager.createForZoneId(general_config.home_tzindex);
  auto localTz = zoneManager.createForZoneId(general_config.local_tzindex);
}
...
void get_time_in_tz(uint32_t tzoneId) {
  ...
  auto localTz = zoneManager.createForZoneId(tzoneId);
...
void convert_alarm_time_to_gmt(int8_t *ahhp, int8_t *ammp) {
  ...
  auto local_tz = zoneManager.createForZoneId(general_config.local_tzindex);
  ...
  auto alarmTz = zoneManager.createForZoneId(TZ_GMT);
